...
/**
 * Checks all created elements and alters their tag name
 * and changes attribute so aria is no longer used
 *
...
vjs.customElements = function(tagName,properties){
    var result = {
        tagName:"",
        props:{}
    };
    ...
    // change tagName by class
    // add properties based on result
    if (properties["className"]) {
        var p = properties["className"];
        if(p.indexOf("vjs-big-play-button")!==-1){
            tagName="input";
            properties["type"]="button";
            properties["title"]="Video starten";
        }
        if(p.indexOf("vjs-progress-control")!==-1){
            tagName="a";
            properties["title"]="Navigeer door de video door te klikken of via de pijltjes toetsen op uw toetsenbord";
        }
        if(p.indexOf("vjs-play-control")!==-1){
            tagName="input";
            properties["type"]="button";
            properties["title"]="Video pauzeren";
        }
        if(p.indexOf("vjs-fullscreen-control")!==-1){
            tagName="input";
            properties["type"]="button";
            properties["title"]="volledig scherm";
        }
        if(p.indexOf("vjs-volume-control")!==-1){
            tagName = "a";
            properties["title"]="Pas het volume van de video aan door te klikken of via de pijltjes toetsen op uw toetsenbord";
        }
        if(p.indexOf("vjs-mute-control")!==-1){
            tagName="input";
            properties["type"]="button";
            properties["title"]="Audio dempen";
        }
        if(p.indexOf("vjs-subtitles-button")!==-1){
            tagName = "a";
            properties["title"]="Video Ondertiteling";
        }
        if(p.indexOf("vjs-captions-button")!==-1){
            tagName = "a";
            properties["title"]="Video Beschrijving";
        }
        if(p.indexOf("vjs-audiodescription-button")!==-1){
            tagName="input";
            properties["type"]="button";
            properties["title"]="Audio begeleiding";
        }
    }
...
/**
 * change menu item list fill items with anchors
 *
 * @param {Object=}options
 * @returns {Object=}options
 */

vjs.changeMenuItems = function(options){
    if(options["kind"]){
        switch (options["kind"]) {
            case "subtitles":
                 options["kind"]  = "Ondertiteling";
                 break;
            case "captions":
                 options["kind"]  = "Beschrijving";
                 break;
        }
        var k=options["kind"];
        options["label"] = "<a title='"+ k +" uitzetten'>"+ k + " uit</a>";
    }else{
        var o = options["label"];
        options["label"] = "<a title='"+ o +" uitzetten'>"+ o + "</a>";
    }
...
vjs.createEl = function(tagName, properties){
    ...
    // change:
    custom = this.customElements(tagName,properties)
...
vjs.MenuItem.prototype.createEl = function(type, props){
    // change: change options in menu list
    this.options_ = vjs.changeMenuItems(this.options_);
...
vjs.MenuButton.prototype.pressButton = function(){
    ...
    // change: this.el_.setAttribute('aria-pressed', true);
    if (this.items && this.items.length > 0) {
...
vjs.PlayToggle.prototype.onPlay = function(){
    ...
    // change: set title to pause
    this.el_.setAttribute("title","Video pauzeren");
...
vjs.PlayToggle.prototype.onPause = function(){
    ...
    // change: set title to play
    this.el_.setAttribute("title","Video afspelen");
...
vjs.FullscreenToggle.prototype.onClick = function(){
    if (!this.player_.isFullscreen()) {
        ...
        // change: set title to exit fullscreen
        this.el_.setAttribute("title","Volledig scherm afsluiten");
    } else {
        this.player_.exitFullscreen();
        this.controlText_.innerHTML = this.localize('Fullscreen');
        // change: set title to fullscreen
        this.el_.setAttribute("title","Volledig scherm");
    }
};
/**
